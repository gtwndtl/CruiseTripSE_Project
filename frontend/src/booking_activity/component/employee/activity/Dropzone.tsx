...
interface DropzoneProps {
    activityPicURL?: string; // Optional prop for image URL
    onDrop: (files: File[]) => void;
}

const Dropzone: React.FC<DropzoneProps> = ({ activityPicURL, onDrop }) => {
    ...
    return (
        ...
        >
            <input {...getInputProps()} />
            {activityPicURL || files.length > 0 ? (
                <div style={{ width: "100%", height: "100%", position: "relative" }}>
                    <img
                        src={activityPicURL || files[0].preview}
                        alt="Preview"
                        style={{
                            width: "100%",
                            height: "100%",
