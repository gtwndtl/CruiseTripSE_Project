...
import { GetGuestByID } from "../../../booking_trip/service/https";
...
export default function FoodReceipt() {
  ...
  const getFoodServicePaymentById = async () => {
    ...
    if (res.status === 200) {
      ...
      getGuestByID(res.data.Order.GuestID);
    } else {
      messageApi.open({
        type: "error",
        content: res.data.error,
  ...
  // useEffect(() => {
  //   // Set isLoaded when all data is ready (foodServicePayment, Guest, orderDetails)
  //   if (foodServicePayment && Guest && orderDetail.length > 0) {
  //     setIsLoaded(true);
  //   }
  // }, [foodServicePayment, Guest, orderDetail]);
      ...
      <div className="guest-detail-container">
        <div className="guest-detail">
          ...
          <div className="guest-info">
            ...
            <span className="address">{guest?.Address}</span>
          </div>
        </div>
