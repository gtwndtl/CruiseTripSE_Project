...
import { GetBookingTripByID } from "../../../booking_trip/service/https";
...
function TripReceipt() {
  ...
  const getBookingTrip = async (booking_trip_id: number) => {
    const res = await GetBookingTripByID(String(booking_trip_id));
    // console.log("getBookingTrip", res)
    if (res.status === 200) {
      // setBookingTrip(res.data);
      setCruiseTrip(res.data.CruiseTrip)
  ...
  const getBookingCabin = async (booking_cabin_id: number) => {
    const res = await GetBookingCabinByID(String(booking_cabin_id));
    if (res.status === 200) {
          ...
          <tr>
            ...
            <td className="trip-detail">
              <span>
                ...
                <p>
                  {cruiseTrip?.CruiseTripName}{" "}
                  {cruiseTrip?.ParticNum} Night
                </p>
              </span>
            </td>
            <td className="amount">{formatPriceWithTwoDecimals(cruiseTrip?.PlanPrice ?? 0)}</td>
          </tr>
          <tr>
            ...
            <td className="cabin-detail">
              <span>
                ...
                <p>Cabin Number: {cabin?.CabinNumber}</p>
          ...
          <tr>
            ...
            <td>
              {formatPriceWithTwoDecimals(
                (tripPayment?.TotalPrice ?? 0) - (discountTripAmount ?? 0)
              )}
            </td>
          </tr>
        </tbody>
